...
public:
  ...
  void addType(unsigned int type) { type_ |= type; }
  void removeType(unsigned int type) { type_ &= ~type; }
  float BValue() const { return BValue_; }
  void setBValue(float bvalue) { BValue_ = bvalue; }
  float occ() const { return occ_; }
  void setOcc(float occ) { occ_ = occ; }
  short color() const { return color_; }
  void setColor(short c) { color_ = c; }
  ...
  void addDelta(float dx, float dy, float dz) { dx_ += dx; dy_ += dy; dz_ += dz; }
  void resetDelta() { dx_ = 0.0f; dy_ = 0.0f; dz_ = 0.0f; }
  float weight() const { return weight_; }
  void addWeight(float w) { weight_ += w; }
  void resetWeight() { weight_ = 0.0f; }
  unsigned char radius_type() const { return radius_type_; }
  void set_radius_type(unsigned char c) { radius_type_ = c; }

  int symmop() const { return symmop_; }
  void setSymmop(int i) { symmop_ = i; }

  void copyChemicalData(const MIAtom& source);

  char altloc() const { return altloc_; }
  void setAltloc(char c) { altloc_ = c; }
  int atomnumber() const { return atomnumber_; }
  void setAtomnumber(int n) { atomnumber_ = n; }
  short atomicnumber() const { return atomicnumber_; }
  void setAtomicnumber(short n) { atomicnumber_ = n; }
  int isaromatic() const { return isaromatic_; }
  void setIsaromatic(int i) { isaromatic_ = i; }
  int hybrid() const { return hybrid_; }
  void setHybrid(int i) { hybrid_ = i; }
  int geom() const { return geom_; }
  void setGeom(int i) { geom_ = i; }
  int mass() const { return mass_; }
  void setMass(int m) { mass_ = m; }
  int formal_charge() const { return formal_charge_; }
  void set_formal_charge(int i) { formal_charge_ = i; }
  float charge() const { return charge_; }
  void setCharge(float c) { charge_ = c; }
  int hcount() const { return hcount_; }
  void setHcount(int i) { hcount_ = i; }
  int iscyclic() const { return iscyclic_; }
  void setIscyclic(int i) { iscyclic_ = i; }
  int ring_system() const { return ring_system_; }
  void set_ring_system(int i) { ring_system_ = i; }
  int smallest_aromatic_ring() const { return smallest_aromatic_ring_; }
  void set_smallest_aromatic_ring(int i) { smallest_aromatic_ring_ = i; }
  int search_flag() const { return search_flag_; }
  void set_search_flag(int i) { search_flag_ = i; }

  const MIAtomList& nabors() const { return nabors_; }
  void addNabor(MIAtom* atom) { nabors_.push_back(atom); }
  const std::vector<int>& bondnumbers() const { return bondnumbers_; }
  void addBondnumber(int bondNumber) { bondnumbers_.push_back(bondNumber); }
