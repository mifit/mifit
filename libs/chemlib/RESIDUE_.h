...
public:
  ...
  RESIDUE* next() const { return next_res; }
  void setNext(RESIDUE* r) { next_res = r; }
  RESIDUE* prev() const { return prev_res; }
  void setPrev(RESIDUE* r) { prev_res = r; }


  const std::vector<Bond>& prefBonds() const { return prefbonds; }
  void addPrefBond(const Bond& b) { prefbonds.push_back(b); }
  void setPrefBonds(const std::vector<Bond>& bonds) { prefbonds = bonds; }
  void clearPrefBonds() { prefbonds.clear(); }

  const std::vector<ANGLE>& prefAngles() const { return prefangles; }
  void addPrefAngle(const ANGLE& a) { prefangles.push_back(a); }
  void setPrefAngles(const std::vector<ANGLE>& angles) { prefangles = angles; }
  void clearPrefAngles() { prefangles.clear(); }
