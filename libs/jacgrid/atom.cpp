...
JACAtomsXYZR::JACAtomsXYZR(const float *xyzr,
  ...
  _xyzr(xyzr),_count(count),_selected(selected),_sel_mask(sel_mask) {}


unsigned int JACAtomsXYZR::GetCount() const { return _count; }
...
JACAtomsXYZROTE::JACAtomsXYZROTE(const float *xyzr,
                                 ...
                                 const unsigned int *selected,
                                 unsigned int sel_mask)
  : JACAtomsXYZR(xyzr,count,selected,sel_mask),_occupancy(occupancy),
    _bfactor(bfactor),_element(element)
{}
