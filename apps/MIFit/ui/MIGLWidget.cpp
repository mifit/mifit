...
class MyMIMolOptCheckPoint : public MIMolOptCheckPoint {
public:
  MyMIMolOptCheckPoint(MIGLWidget* parent,
                       ViewPoint* viewpoint) : _parent(parent),
...
MIGLWidget::MIGLWidget() : MIEventHandler(MIMainWindow::instance()) {
  ...
  switch (nChar) {
    ...
    case Qt::Key_Left: // left arrow
      if (xfit_mouse && IsFitting()) {
        ...
        } else {RightMouseDrag(CPoint(-10, 0), 0.0F, angle, 0.0F);}
      } else {
        if (control) {
          if (shift) {
            RightMouseDrag(CPoint(-1, 0), 0.0F, fineangle, 0.0F);
          ...
          } else {viewpoint->rotate(0.0F, angle, 0.0F);}
        }
      }
      ReDraw();
      handled = true;
    ...
    case Qt::Key_Up:  // up arrow
      if (xfit_mouse && IsFitting()) {
        ...
        } else {RightMouseDrag(CPoint(0, -2), -angle, 0.0F, 0.0F);}
      } else {
        if (control && shift) {
          ...
          } else {viewpoint->rotate(-angle, 0.0F, 0.0F);}
        }
      }
      ReDraw();
      handled = true;
...
bool MIGLWidget::FitOK() {
  if (CurrentAtoms.size() > 0) {
    ...
    switch (ret) {
      case MI_CANCEL: return false; break;
      case MI_YES:    ApplyFit(); break;
      case MI_NO:     cancelFitting(); break;
    }
  }
  return true;
}
...
#define cmdslider(panel, factor, special) \
  ...
  if (!strncasecmp(buf, "label ", 6) || !strncasecmp(buf, "label\t", 6) ) {
    ...
    } else {sscanf(colorstring, "%d", &color);
