...
class MISurface : public surfaceT {
  public:
    MISurface() : _atom_count(0), _hash(0.0f), _valid(0) {}

    MISurface(unsigned int surface_type,
              const std::vector<Molecule*>& mols,
              unsigned int count,
              const unsigned int* selected,
              unsigned int mask = 1) : _atom_count(0), _hash(0.0f), _valid(0) {
...
class SuperSurface : public MISurface {
  ...
  public:
    ...
    unsigned int Count() { return surfaces.size(); }
