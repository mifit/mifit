...
Molecule::Molecule(int type) : MIMoleculeBase(), mapheader(new CMapHeader), m_pSecondaryStructure(NULL),
  secStruc_ribbon(false), secStruc_schematic(false) {
  ModelType = type;
  ...
  ribbon_coloring = 0;
  undoable = false;
  s_main = s_sides = s_waters = s_nonprotein = 1; s_link = 0; // the default startup settings
  srfdotsper = 0.5;
  srfboxsize = 3.0;
  symm_center[0] = 0;
  symm_center[1] = 0;
...
Molecule::Molecule(RESIDUE* reslist, std::string cmpd, FILE* fp, Bond* conns, int nconns, int type)
  : MIMoleculeBase(reslist, cmpd.c_str(), conns, nconns), mapheader(new CMapHeader), m_pSecondaryStructure(NULL),
  secStruc_ribbon(false), secStruc_schematic(false) {
  ModelType = type;
  visible = dots_visible = labels_visible = annots_visible = 1;
...
void Molecule::Select(int clear, int main, int sides, int nonprotein,
  ...
  for (MIIter<RESIDUE> res = GetResidues(); Residue::isValid(res); ++res) {
    ...
    if (!resshow.empty() && inrange) {
      ...
      } else {strcat(buf, res->name().c_str());}
      strcat(buf, ",");
      ...
      if (CA1 && O1 && C1) {
        ...
        if (res->next()) {
          if ((p1->occ() == 'S' || p1->occ() == 'H')
            ...
            } else {color = secstrcolor(res->secstr());}
            p1->setColor(color);
            p2->setColor(color);
            p3->setColor(color);
            p4->setColor(color);
...
int Molecule::SaveSymmMolecule(MIAtom* symatom, FILE* fp) {
  ...
  if (isym > mapheader->nsym) {
    Logger::message("bad symm atom"); return 0;
  }
